...
public class MasterNodeStopJob implements Handler<SfsRequest> {

    private static final Logger LOGGER = getLogger(MasterNodeStopJob.class);
    ...
    public void handle(SfsRequest httpServerRequest) {
        ...
        Defer.aVoid()
                ...
                .flatMap(aVoid -> {
                    ...
                    return Defer.aVoid()
                            .flatMap(aVoid1 -> jobs.stop(vertxContext, jobId))
                            .flatMap(aVoid1 -> {
                                if (timeout >= 0) {
