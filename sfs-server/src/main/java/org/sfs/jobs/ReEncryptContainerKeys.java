...
public class ReEncryptContainerKeys extends AbstractJob {

    private static final Logger LOGGER = LoggerFactory.getLogger(ReEncryptContainerKeys.class);


    @Override
    public String id() {
        return Jobs.ID.RE_ENCRYPT_CONTAINER_KEYS;
    }
    ...
    protected Observable<Void> execute0(VertxContext<Server> vertxContext) {
        return Defer.aVoid()
                .flatMap(index -> {
                    ...
                    return containerKeys.maintain(vertxContext);
                });
    }
}
