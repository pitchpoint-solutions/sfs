...
public class MaintainContainerObjects implements Func1<Void, Observable<Void>> {

    private static final Logger LOGGER = getLogger(MaintainContainerObjects.class);
    private final VertxContext<Server> vertxContext;
    private final String containerId;

    public MaintainContainerObjects(VertxContext<Server> vertxContext, String containerId) {
    ...
    public static Observable<Void> repair(VertxContext<Server> vertxContext, String containerId) {
        return Defer.aVoid()
                .flatMap(new MaintainContainerObjects(vertxContext, containerId))
                .map(new ToVoid<>());
    }
}
