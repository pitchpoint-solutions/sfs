...
public class ValidateActionObjectCreate implements Func1<TransientVersion, Observable<TransientVersion>> {
    ...
    public Observable<TransientVersion> call(TransientVersion version) {
        ...
        return authProvider.canObjectCreate(sfsRequest, version)
                .map(canDo -> {
                    if (!canDo) {
                        JsonObject jsonObject = new JsonObject()
                                .put("message", "Object Create Forbidden");
