...
public class ValidateActionContainerDelete implements Func1<PersistentContainer, Observable<PersistentContainer>> {
    ...
    public Observable<PersistentContainer> call(PersistentContainer container) {
        ...
        return authProvider.canContainerDelete(sfsRequest, container)
                .map(canDo -> {
                    if (!canDo) {
                        JsonObject jsonObject = new JsonObject()
                                .put("message", "Container Delete Forbidden");
