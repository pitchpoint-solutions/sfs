...
public class ValidateActionObjectUpdate implements Func1<TransientVersion, Observable<TransientVersion>> {
    ...
    public Observable<TransientVersion> call(TransientVersion version) {
        ...
        return authProvider.canObjectUpdate(sfsRequest, version)
                .map(canDo -> {
                    if (!canDo) {
                        JsonObject jsonObject = new JsonObject()
                                .put("message", "Object Update Forbidden");
