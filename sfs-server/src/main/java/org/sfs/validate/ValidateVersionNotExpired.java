...
public class ValidateVersionNotExpired implements Func1<TransientVersion, TransientVersion> {
    ...
    public TransientVersion call(TransientVersion transientVersion) {
        ...
        if (oDeleteAt.isPresent()) {
            ...
            long deletedAt = oDeleteAt.get();

            if (currentTimeMillis() >= deletedAt) {
                JsonObject jsonObject = new JsonObject()
                        .put("message", format("Version %d is expired", transientVersion.getId()));
