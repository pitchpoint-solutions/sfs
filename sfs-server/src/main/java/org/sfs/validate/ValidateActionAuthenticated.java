...
public class ValidateActionAuthenticated implements Func1<Void, Observable<Void>> {
    ...
    public Observable<Void> call(Void aVoid) {
        ...
        return authProvider.isAuthenticated(sfsRequest)
                .map(isAuthenticated -> {
                    if (!isAuthenticated) {
                        JsonObject jsonObject = new JsonObject()
                                .put("message", "Action Forbidden");
