...
public class ValidateVersionNotDeleteMarker implements Func1<TransientVersion, TransientVersion> {

    @Override
    public TransientVersion call(TransientVersion transientVersion) {
        if (TRUE.equals(transientVersion.getDeleteMarker())) {

            JsonObject jsonObject = new JsonObject()
                    .put("message", format("Version %d is delete marker", transientVersion.getId()));
