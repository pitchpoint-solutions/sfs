...
public class ValidateActionObjectRead implements Func1<TransientVersion, Observable<TransientVersion>> {
    ...
    public Observable<TransientVersion> call(TransientVersion version) {
        ...
        return authProvider.canObjectRead(sfsRequest, version)
                .map(canDo -> {
                    if (!canDo) {
                        JsonObject jsonObject = new JsonObject()
                                .put("message", "Object Read Forbidden");
