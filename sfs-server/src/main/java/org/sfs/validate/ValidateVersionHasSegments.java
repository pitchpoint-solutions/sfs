...
public class ValidateVersionHasSegments implements Func1<TransientVersion, TransientVersion> {

    @Override
    public TransientVersion call(TransientVersion transientVersion) {
        Optional<Long> length = transientVersion.getContentLength();
        if (transientVersion.getSegments().isEmpty()
                && length.isPresent()
                && length.get() > 0) {

            JsonObject jsonObject = new JsonObject()
                    .put("message", format("Version %d has no segments", transientVersion.getId()));
