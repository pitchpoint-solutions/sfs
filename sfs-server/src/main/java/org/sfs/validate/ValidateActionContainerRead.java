...
public class ValidateActionContainerRead implements Func1<PersistentContainer, Observable<PersistentContainer>> {
    ...
    public Observable<PersistentContainer> call(PersistentContainer container) {
        ...
        return authProvider.canContainerRead(sfsRequest, container)
                .map(canDo -> {
                    if (!canDo) {
                        JsonObject jsonObject = new JsonObject()
                                .put("message", "Container Read Forbidden");
