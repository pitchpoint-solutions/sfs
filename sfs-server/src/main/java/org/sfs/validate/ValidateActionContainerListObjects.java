...
public class ValidateActionContainerListObjects implements Func1<PersistentContainer, Observable<PersistentContainer>> {
    ...
    public Observable<PersistentContainer> call(PersistentContainer container) {
        ...
        return authProvider.canContainerListObjects(sfsRequest, container)
                .map(canDo -> {
                    if (!canDo) {
                        JsonObject jsonObject = new JsonObject()
                                .put("message", "Container List Objects Forbidden");
