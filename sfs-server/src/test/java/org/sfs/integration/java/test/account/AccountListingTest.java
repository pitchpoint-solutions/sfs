...
public class AccountListingTest extends BaseTestVerticle {
    ...
    public void testListAccountObjects(TestContext context) {
        ...
        prepareAccount(context)
                .flatMap(new PutContainer(HTTP_CLIENT, accountName, containerName1, authNonAdmin))
                .map(new HttpClientResponseHeaderLogger())
                .map(new AssertHttpClientResponseStatusCode(context, HTTP_CREATED))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutContainer(HTTP_CLIENT, accountName, containerName2, authNonAdmin))
                .map(new HttpClientResponseHeaderLogger())
                .map(new AssertHttpClientResponseStatusCode(context, HTTP_CREATED))
                .map(new ToVoid<HttpClientResponse>())
                // put three objects then list and assert
                .flatMap(new PutObject(HTTP_CLIENT, accountName, containerName1, objectName + "/4", authNonAdmin, new byte[]{}))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutObject(HTTP_CLIENT, accountName, containerName1, objectName + "/3", authNonAdmin, data2)
                        .setHeader(CONTENT_TYPE, PLAIN_TEXT_UTF_8.toString()))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutObject(HTTP_CLIENT, accountName, containerName1, objectName + "/2", authNonAdmin, data1))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutObject(HTTP_CLIENT, accountName, containerName1, objectName + "/1", authNonAdmin, data0))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutObject(HTTP_CLIENT, accountName, containerName2, objectName + "/4", authNonAdmin, new byte[]{}))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutObject(HTTP_CLIENT, accountName, containerName2, objectName + "/3", authNonAdmin, data2)
                        .setHeader(CONTENT_TYPE, PLAIN_TEXT_UTF_8.toString()))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutObject(HTTP_CLIENT, accountName, containerName2, objectName + "/2", authNonAdmin, data1))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new RefreshIndex(HTTP_CLIENT, authAdmin))
                .flatMap(new GetAccount(HTTP_CLIENT, accountName, authAdmin)
    ...
    public void testListAccountQueryParamsObjects(TestContext context) {
        Async async = context.async();
        prepareAccount(context)
                .flatMap(new PutContainer(HTTP_CLIENT, accountName, "apples", authNonAdmin))
                .map(new HttpClientResponseHeaderLogger())
                .map(new AssertHttpClientResponseStatusCode(context, HTTP_CREATED))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutContainer(HTTP_CLIENT, accountName, "apricots", authNonAdmin))
                .map(new HttpClientResponseHeaderLogger())
                .map(new AssertHttpClientResponseStatusCode(context, HTTP_CREATED))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutContainer(HTTP_CLIENT, accountName, "bananas", authNonAdmin))
                .map(new HttpClientResponseHeaderLogger())
                .map(new AssertHttpClientResponseStatusCode(context, HTTP_CREATED))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutContainer(HTTP_CLIENT, accountName, "kiwis", authNonAdmin))
                .map(new HttpClientResponseHeaderLogger())
                .map(new AssertHttpClientResponseStatusCode(context, HTTP_CREATED))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutContainer(HTTP_CLIENT, accountName, "oranges", authNonAdmin))
                .map(new HttpClientResponseHeaderLogger())
                .map(new AssertHttpClientResponseStatusCode(context, HTTP_CREATED))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new PutContainer(HTTP_CLIENT, accountName, "pears", authNonAdmin))
                .map(new HttpClientResponseHeaderLogger())
                .map(new AssertHttpClientResponseStatusCode(context, HTTP_CREATED))
                .map(new ToVoid<HttpClientResponse>())
                .flatMap(new RefreshIndex(HTTP_CLIENT, authAdmin))
